...
const { catchRevert } = require("../src/utils/exceptions");
...
contract("CourseMarketplace", (accounts) => {
  ...
  describe("Purchase the new course", () => {
    ...
    it("can get the purchased course hash by index", async () => {
      ...
      assert.equal(
        courseHash,
        expectedHash,
        "Couese hash is not matching the purchased one"
      );
    });
    ...
    it("should match the purchased data of the course by buyer", async () => {
      ...
      assert.equal(course.id, expectedIndex, "Couse index should be 0");
      assert.equal(course.price, value, `Course price shuld be ${value}`);
  ...
  describe("Activate the purchased course", () => {
    it("should NOT be able to actvate course by NOT contract owner", async () => {
  ...
  describe("Transfer ownership", () => {
    ...
    it("should transfer ownership back to initial contract owner", async () => {
      await _contract.transferOwnership(currentOwner, { from: accounts[2] });
      const owner = await _contract.getContractOwner();
